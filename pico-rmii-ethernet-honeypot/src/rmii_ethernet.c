...
// KZY: This works, however packet detection should be done in main.
// However it is not possible as if we use httpd_init() in lwip as
// we would then have to create our own WebServer to handle all incoming
// TCP connections.

// To test the function below 
// Include httpd_init() in main
// and then run the following:
// curl -x OPTIONS -I http://192.168.1.100/   
...
#define TCP_HEADER_SIZE 20 // Standard minimal TCP header size without options (could vary)
    ...
    if (rx_frame_length) {
        ...
        for (int i = 0; i <= rx_frame_length - 8; i++) {
            // Detect the "OPTIONS " pattern, starting from where the TCP payload should be
            if (i >= ETHERNET_HEADER_SIZE + IP_HEADER_SIZE + TCP_HEADER_SIZE &&
                ...
                break; // No need to continue once OPTIONS is detected
            }
        }
