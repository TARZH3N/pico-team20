...
#define PICO_RMII_ETHERNET_SM_RX    (rmii_eth_netif_config.pio_sm_start)        // 0
#define PICO_RMII_ETHERNET_SM_TX    (rmii_eth_netif_config.pio_sm_start + 1)    // 1
#define PICO_RMII_ETHERNET_RX_PIN   (rmii_eth_netif_config.rx_pin_start)        // 6
#define PICO_RMII_ETHERNET_TX_PIN   (rmii_eth_netif_config.tx_pin_start)        // 10
#define PICO_RMII_ETHERNET_MDIO_PIN (rmii_eth_netif_config.mdio_pin_start)      // 14
#define PICO_RMII_ETHERNET_MDC_PIN  (rmii_eth_netif_config.mdio_pin_start + 1)  // 15
...
// #define IP_HEADER_SIZE 20 // Standard minimal IP header size without options
// #define IP_HEADER_SRC_IP_OFFSET 12 // Offset for source IP in IP header
// #define TCP_HEADER_SIZE 20 // Standard minimal TCP header size without options (could vary)

    if (rx_frame_length) {

        // ZY:
        // This works however packet detection should not be done in driver
        // There should be a callback function to capture raw packets.
        // The problem is that I cannot assign a new HTTP PCB in main to listen to HTTP Traffic
        // As we are using httpd_init() by LWIP.

        // Receiving function should be called in main, snprintf into a buffer to be sent over SPI to listener pico.

        // bool options_detected = false;
        // for (int i = 0; i <= rx_frame_length - 8; i++) {
        //     if (i >= ETHERNET_HEADER_SIZE + IP_HEADER_SIZE + TCP_HEADER_SIZE &&
        //         rx_frame[i] == 'O' && rx_frame[i + 1] == 'P' &&
        //         rx_frame[i + 2] == 'T' && rx_frame[i + 3] == 'I' &&
        //         rx_frame[i + 4] == 'O' && rx_frame[i + 5] == 'N' &&
        //         rx_frame[i + 6] == 'S' && rx_frame[i + 7] == ' ') {
        //         options_detected = true;
        //         break; // Once options detected just break
        //     }
        // }
        ...
        //     for (int i = 0; i < 4; i++) {
        //         src_ip[i] = rx_frame[ETHERNET_HEADER_SIZE + IP_HEADER_SRC_IP_OFFSET + i];
        //     }

        //     printf("OPTIONS DETECTED FROM %u.%u.%u.%u\n",
